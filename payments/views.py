...
def initialize_payment(request, booking_id):
    ...
    IMPORTANT:
    Amount must come from the BOOKING SNAPSHOT, not the live service price.
    """
    try:
        booking = Booking.objects.get(id=booking_id)
    except Booking.DoesNotExist:
...
def payment_success(request):
    """
    User returns from Flutterwave after paying.
    """
    reference = request.GET.get("tx_ref") or request.GET.get("reference")
...
def flutterwave_webhook(request):
    """
    Flutterwave webhook verification (server → server).
    """
    if request.method != "POST":
        return HttpResponse("Invalid method", status=405)
...
def check_payment_status(request, reference):
    """
    Optional endpoint used by frontend/mobile apps.
    """
    try:
        txn = Transaction.objects.get(reference=reference)
        booking = txn.booking
    ...
    return JsonResponse({
        ...
        "booking_status": booking.status,
        "payment_status": booking.payment_status,
        "booked_total_price": float(booking.booked_total_price),
    })